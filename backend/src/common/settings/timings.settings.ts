...
import { EPHEMERAL_REPLY_DURATION_LONG, WAR_START_INTERVAL } from '../constant';
...
export class TimingsSettings extends BaseSetting<ButtonBuilder> {
	...
	public getSettingsRows(
		...
		const row = new ActionRowBuilder<ButtonBuilder>().addComponents(customTimingsButton);
	...
	public async getCurrentSettings(guildData: DBGuild) {
		...
		if (!guildData.customTimings) {
			...
			await guildData
				...
				.then(() => {
					resetMessage =
						'The saved respawn timers had invalid entries and have been reset to default';
				})
				.catch(() => {
		...
		return `${chunks
			.map(
				(chunk) =>
					`\`\`\`brainfuck\n${chunk
						.map((val) => (!TimingsSettings.DEFAULT.includes(val) ? `[${val}]` : val))
						.join(' > ')}\`\`\``
			)
			.join('')}${resetMessage ? '\n' + resetMessage : ''}`;
	}
	public showModal(interaction: ButtonInteraction, timings?: string) {
		...
		const timingsInput = new TextInputBuilder()
			...
			.setValue(
				timings
					? TimingsSettings.sort(timings).join(', ')
					: [...TimingsSettings.DEFAULT].sort().reverse().join(', ')
			);
		const apiKeyRow = new ActionRowBuilder<TextInputBuilder>().addComponents(timingsInput);
	...
	public async onInteract(
		...
		switch (option) {
			case ETimingsSettingsOptions.TIMINGS:
				...
				this.waitingForTimingsModal = false;
				const timings = modalInteraction.fields.getTextInputValue(
					this.getCustomId(ESettingsID.TIMINGS, [ETimingsSettingsOptions.TIMINGS])
				);
				const [validSyntax, reason] = TimingsSettings.checkSyntax(timings);
				if (!validSyntax) {
					await modalInteraction
						.reply({ ephemeral: true, content: reason })
	...
	public static equalsDefault(text: string): boolean {
		return (
			TimingsSettings.sort(text).sort().toString() ===
			[...TimingsSettings.DEFAULT].sort().toString()
		);
	}
	...
	public static convertToRespawnData(timers: number[]): WarInfo {
		...
		start.setMinutes(Math.floor(start.getMinutes() / WAR_START_INTERVAL) * WAR_START_INTERVAL);
		start.setSeconds(0);
		start.setMilliseconds(0);
		const now = new Date();

		const timePassedSeconds = Math.floor((now.getTime() - start.getTime()) / 1000);
		const timeLeftTotalSeconds = WAR_START_INTERVAL * 60 - timePassedSeconds;
		...
		const duration = current ? (prev ? prev - current : WAR_START_INTERVAL * 60 - current) : -1;
