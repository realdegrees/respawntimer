...
export class RaidhelperSettings extends BaseSetting<ButtonBuilder | StringSelectMenuBuilder> {
	...
	public async getSettingsRows(
		...
		const defaultVoiceChannelRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
			voiceChannelSelectMenu.getMenu()
		);
		const raidhelperEventChannelRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
			textChannelSelectMenu.getMenu()
		);

		return Promise.resolve([raidhelperEventChannelRow, defaultVoiceChannelRow, buttonRow]);
	}
	public async getCurrentSettings(dbGuild: DBGuild) {
		...
		const scheduledEvents =
			events.length > 0 ? formatEventsNoFetch(dbGuild, ...events) : ['*None*'];
		...
		const apiKeyValidText = apiKey
			...
			: '';
		const apiKeyText = apiKey ? '||```fix\n' + apiKey + '```||' : '```diff\n- Not Set```';
		const apiKeyNotSetInfo = !apiKey
			? "Use `/apikey show` to retrieve your Raidhelper API Key\nOr `/apikey refresh` if you don't have an API Key yet"
			: '';
		const apiKeyInvalidInfo =
			apiKey && !apiKeyValid
				? 'You can use `/apikey refresh` to get a new API Key and enter it again'
				: '';
		...
		return [
			...
			},
			{
				name: `Auto-Widget`,
				value:
					'*The bot will automatically start the text-widget when an event starts*\n' +
					(dbGuild.raidHelper.widget ? '```diff\n+ Enabled ```' : '```diff\n- Disabled ```'),
				inline: true
			},
			{
	...
	public showModal(interaction: ButtonInteraction) {
		...
		const apiKeyRow = new ActionRowBuilder<TextInputBuilder>().addComponents(apiKey);
	...
	public async onInteract(
		...
		switch (option) {
			case ERaidhelperSettingsOptions.API_KEY:
				...
				this.waitingForApiKeyModal = false;
				const apiKey = modalInteraction.fields.getTextInputValue(
					this.getCustomId(ESettingsID.RAIDHELPER, [ERaidhelperSettingsOptions.API_KEY])
				);
