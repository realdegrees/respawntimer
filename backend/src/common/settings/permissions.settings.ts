import { ActionRowBuilder, AnySelectMenuInteraction, ButtonInteraction, ButtonStyle, CacheType, EmbedField, Guild, Interaction, MessageComponentInteraction, ModalSubmitInteraction, RoleSelectMenuBuilder, RoleSelectMenuInteraction } from 'discord.js';
...
export class PermissionSettings extends BaseSetting<RoleSelectMenuBuilder> {
    ...
    public async getCurrentSettings(guildData: DBGuild, guild?: Guild | undefined) {
        const editorRoles = await Promise.all(guildData.editorRoleIDs.map((id) => guild?.roles.fetch(id)))
            .then((roles) =>
                roles.filter((role) => !!role)
                    .map((role) => `${role}`)
                    .join('\n')
            ).catch(() => ['Unable to retrieve roles']);
        const assistantRoles = await Promise.all(guildData.assistantRoleIDs.map((id) => guild?.roles.fetch(id)))
            .then((roles) =>
                roles.filter((role) => !!role)
                    .map((role) => `${role}`)
                    .join('\n')
